...
-- +------------------------------------------------------------------------------------------------------+
-- | Create the security roles for PROD2_EDW_SYSADMIN and PROD2_EDW_SECADMIN                              |
-- +------------------------------------------------------------------------------------------------------+

   USE ROLE USERADMIN;

   CREATE ROLE IF NOT EXISTS PROD2_EDW_SECADMIN;
   CREATE ROLE IF NOT EXISTS PROD2_EDW_SYSADMIN;

   GRANT CREATE ROLE ON ACCOUNT TO PROD2_EDW_SECADMIN;
   GRANT ROLE PROD2_EDW_SECADMIN TO ROLE USERADMIN;
   GRANT ROLE PROD2_EDW_SYSADMIN TO ROLE SYSADMIN;

   USE ROLE SYSADMIN;

   GRANT CREATE DATABASE ON ACCOUNT TO PROD2_EDW_SYSADMIN;
   GRANT CREATE WAREHOUSE ON ACCOUNT TO PROD2_EDW_SYSADMIN;
...
-- +------------------------------------------------------------------------------------------------------+
-- | Try to use role PROD2_EDW_SYSADMIN                                                                   |
-- +------------------------------------------------------------------------------------------------------+

USE ROLE PROD2_EDW_SYSADMIN;

-- +------------------------------------------------------------------------------------------------------+
-- | Create database PROD2_EDW                                                                            |
-- +------------------------------------------------------------------------------------------------------+

   CREATE DATABASE IF NOT EXISTS PROD2_EDW DATA_RETENTION_TIME_IN_DAYS = 1;
USE DATABASE PROD2_EDW;
...
-- +------------------------------------------------------------------------------------------------------+
-- | Try to use role PROD2_EDW_SECADMIN                                                                   |
-- +------------------------------------------------------------------------------------------------------+

USE ROLE PROD2_EDW_SECADMIN;

-- +------------------------------------------------------------------------------------------------------+
-- | Create Account Level functional roles                                                                |
-- +------------------------------------------------------------------------------------------------------+

   CREATE ROLE IF NOT EXISTS PROD2_EDW_ELT_DEV;
   CREATE ROLE IF NOT EXISTS PROD2_EDW_ANALYST;
   CREATE ROLE IF NOT EXISTS PROD2_EDW_DATA_SCI;
...
USE ROLE PROD2_EDW_SECADMIN;
   CREATE  ROLE IF NOT EXISTS _PROD2_EDW_ADHOC_WH_WU;
   CREATE  ROLE IF NOT EXISTS _PROD2_EDW_ADHOC_WH_WFULL;
   CREATE  ROLE IF NOT EXISTS _PROD2_EDW_ADHOC_WH_ALL;

-- +------------------------------------------------------------------------------------------------------+
-- | Grant ownership on warehouse access roles to PROD2_EDW_SECADMIN                                      |
-- +------------------------------------------------------------------------------------------------------+

   GRANT OWNERSHIP ON ROLE _PROD2_EDW_ADHOC_WH_WU TO ROLE PROD2_EDW_SECADMIN;
   GRANT OWNERSHIP ON ROLE _PROD2_EDW_ADHOC_WH_WFULL TO ROLE PROD2_EDW_SECADMIN;
   GRANT OWNERSHIP ON ROLE _PROD2_EDW_ADHOC_WH_ALL TO ROLE PROD2_EDW_SECADMIN;
...
-- +------------------------------------------------------------------------------------------------------+
-- | Grant account level functional roles to PROD2_EDW_SYSADMIN                                           |
-- +------------------------------------------------------------------------------------------------------+

   GRANT ROLE PROD2_EDW_ELT_DEV TO ROLE PROD2_EDW_SYSADMIN;
   GRANT ROLE PROD2_EDW_ANALYST TO ROLE PROD2_EDW_SYSADMIN;
   GRANT ROLE PROD2_EDW_DATA_SCI TO ROLE PROD2_EDW_SYSADMIN;

-- +------------------------------------------------------------------------------------------------------+
-- | Grant database functional roles to PROD2_EDW_SYSADMIN                                                |
-- +------------------------------------------------------------------------------------------------------+

USE ROLE PROD2_EDW_SYSADMIN;
USE DATABASE PROD2_EDW;
   GRANT DATABASE ROLE ELT_DEV TO ROLE PROD2_EDW_SYSADMIN;
   GRANT DATABASE ROLE ANALYST TO ROLE PROD2_EDW_SYSADMIN;
   GRANT DATABASE ROLE DATA_SCI TO ROLE PROD2_EDW_SYSADMIN;

-- +------------------------------------------------------------------------------------------------------+
-- | Grant database functional roles to account level functional roles                                    |
-- +------------------------------------------------------------------------------------------------------+

   GRANT DATABASE ROLE ELT_DEV TO ROLE PROD2_EDW_ELT_DEV;
   GRANT DATABASE ROLE ANALYST TO ROLE PROD2_EDW_ANALYST;
   GRANT DATABASE ROLE DATA_SCI TO ROLE PROD2_EDW_DATA_SCI;

-- +------------------------------------------------------------------------------------------------------+
-- | Grant access roles to PROD2_EDW_SYSADMIN                                                             |
-- +------------------------------------------------------------------------------------------------------+

   GRANT DATABASE ROLE _RAW_RO TO ROLE PROD2_EDW_SYSADMIN;
   GRANT DATABASE ROLE _RAW_RW TO ROLE PROD2_EDW_SYSADMIN;
   GRANT DATABASE ROLE _RAW_SFULL TO ROLE PROD2_EDW_SYSADMIN;
   GRANT DATABASE ROLE _EDW_RO TO ROLE PROD2_EDW_SYSADMIN;
   GRANT DATABASE ROLE _EDW_RW TO ROLE PROD2_EDW_SYSADMIN;
   GRANT DATABASE ROLE _EDW_SFULL TO ROLE PROD2_EDW_SYSADMIN;
   GRANT DATABASE ROLE _MART_RO TO ROLE PROD2_EDW_SYSADMIN;
   GRANT DATABASE ROLE _MART_RW TO ROLE PROD2_EDW_SYSADMIN;
   GRANT DATABASE ROLE _MART_SFULL TO ROLE PROD2_EDW_SYSADMIN;

-- +------------------------------------------------------------------------------------------------------+
-- | Grant access roles to functional roles                                                               |
-- +------------------------------------------------------------------------------------------------------+

   GRANT DATABASE ROLE _RAW_SFULL TO DATABASE ROLE ELT_DEV;
   GRANT DATABASE ROLE _RAW_RO TO DATABASE ROLE ANALYST;
   GRANT DATABASE ROLE _RAW_RW TO DATABASE ROLE DATA_SCI;
   GRANT DATABASE ROLE _EDW_SFULL TO DATABASE ROLE ELT_DEV;
   GRANT DATABASE ROLE _EDW_RO TO DATABASE ROLE ANALYST;
   GRANT DATABASE ROLE _EDW_RO TO DATABASE ROLE DATA_SCI;
   GRANT DATABASE ROLE _MART_SFULL TO DATABASE ROLE ELT_DEV;
   GRANT DATABASE ROLE _MART_RO TO DATABASE ROLE ANALYST;
   GRANT DATABASE ROLE _MART_RO TO DATABASE ROLE DATA_SCI;

-- +------------------------------------------------------------------------------------------------------+
-- | Grant warehouse access roles to PROD2_EDW_SYSADMIN                                                   |
-- +------------------------------------------------------------------------------------------------------+

USE ROLE PROD2_EDW_SECADMIN;
   GRANT ROLE _PROD2_EDW_ADHOC_WH_WU TO ROLE PROD2_EDW_SYSADMIN;
   GRANT ROLE _PROD2_EDW_ADHOC_WH_WFULL TO ROLE PROD2_EDW_SYSADMIN;
   GRANT ROLE _PROD2_EDW_ADHOC_WH_ALL TO ROLE PROD2_EDW_SYSADMIN;

-- +------------------------------------------------------------------------------------------------------+
-- | Grant virtual warehouse access roles to functional roles                                             |
-- +------------------------------------------------------------------------------------------------------+

   GRANT ROLE _PROD2_EDW_ADHOC_WH_WU TO ROLE PROD2_EDW_ELT_DEV;
   GRANT ROLE _PROD2_EDW_ADHOC_WH_WU TO ROLE PROD2_EDW_ANALYST;
   GRANT ROLE _PROD2_EDW_ADHOC_WH_WU TO ROLE PROD2_EDW_DATA_SCI;
...
-- +------------------------------------------------------------------------------------------------------+
   ...
   GRANT USAGE ON DATABASE PROD2_EDW TO DATABASE ROLE _RAW_RO;
   GRANT USAGE ON DATABASE PROD2_EDW TO DATABASE ROLE _RAW_RW;
   GRANT USAGE ON DATABASE PROD2_EDW TO DATABASE ROLE _RAW_SFULL;
   GRANT USAGE ON DATABASE PROD2_EDW TO DATABASE ROLE _EDW_RO;
   GRANT USAGE ON DATABASE PROD2_EDW TO DATABASE ROLE _EDW_RW;
   GRANT USAGE ON DATABASE PROD2_EDW TO DATABASE ROLE _EDW_SFULL;
   GRANT USAGE ON DATABASE PROD2_EDW TO DATABASE ROLE _MART_RO;
   GRANT USAGE ON DATABASE PROD2_EDW TO DATABASE ROLE _MART_RW;
   GRANT USAGE ON DATABASE PROD2_EDW TO DATABASE ROLE _MART_SFULL;

-- +------------------------------------------------------------------------------------------------------+
-- | Create the warehouses                                                                                |
-- +------------------------------------------------------------------------------------------------------+

   CREATE WAREHOUSE IF NOT EXISTS PROD2_EDW_ADHOC_WH WITH WAREHOUSE_TYPE = 'STANDARD' warehouse_size='xsmall', scaling_policy='standard',min_cluster_count=1, max_cluster_count=1, auto_suspend=300,auto_resume=true, initially_suspended=true;

-- +------------------------------------------------------------------------------------------------------+
-- | Grant privileges on warehouses to access roles                                                       |
-- +------------------------------------------------------------------------------------------------------+

   GRANT USAGE ON WAREHOUSE PROD2_EDW_ADHOC_WH TO ROLE _PROD2_EDW_ADHOC_WH_WU;
   GRANT USAGE, OPERATE,MODIFY ON WAREHOUSE PROD2_EDW_ADHOC_WH TO ROLE _PROD2_EDW_ADHOC_WH_WFULL;
   GRANT ALL ON WAREHOUSE PROD2_EDW_ADHOC_WH TO ROLE _PROD2_EDW_ADHOC_WH_ALL;

-- +------------------------------------------------------------------------------------------------------+
-- | Create the schemas                                                                                   |
-- +------------------------------------------------------------------------------------------------------+

   CREATE SCHEMA IF NOT EXISTS PROD2_EDW.RAW WITH MANAGED ACCESS data_retention_time_in_days=1 comment = '';
   CREATE SCHEMA IF NOT EXISTS PROD2_EDW.EDW WITH MANAGED ACCESS data_retention_time_in_days=1 comment = '';
   CREATE SCHEMA IF NOT EXISTS PROD2_EDW.MART WITH MANAGED ACCESS data_retention_time_in_days=1 comment = '';
...
-- +------------------------------------------------------------------------------------------------------+
-- | Build of PROD2_EDW complete - have a nice day                                                        |
-- +------------------------------------------------------------------------------------------------------+
