...
-- +------------------------------------------------------------------------------------------------------+
-- | Create the security roles for INSTRUCTOR2_DB1_SYSADMIN and INSTRUCTOR2_DB1_SECADMIN                  |
-- +------------------------------------------------------------------------------------------------------+

   USE ROLE USERADMIN;

   CREATE ROLE IF NOT EXISTS INSTRUCTOR2_DB1_SECADMIN;
   CREATE ROLE IF NOT EXISTS INSTRUCTOR2_DB1_SYSADMIN;

   GRANT CREATE ROLE ON ACCOUNT TO INSTRUCTOR2_DB1_SECADMIN;
   GRANT ROLE INSTRUCTOR2_DB1_SECADMIN TO ROLE USERADMIN;
   GRANT ROLE INSTRUCTOR2_DB1_SYSADMIN TO ROLE SYSADMIN;

   USE ROLE SYSADMIN;

   GRANT CREATE DATABASE ON ACCOUNT TO INSTRUCTOR2_DB1_SYSADMIN;
   GRANT CREATE WAREHOUSE ON ACCOUNT TO INSTRUCTOR2_DB1_SYSADMIN;
...
-- +------------------------------------------------------------------------------------------------------+
-- | Try to use role INSTRUCTOR2_DB1_SYSADMIN                                                             |
-- +------------------------------------------------------------------------------------------------------+

USE ROLE INSTRUCTOR2_DB1_SYSADMIN;

-- +------------------------------------------------------------------------------------------------------+
-- | Create database INSTRUCTOR2_DB1                                                                      |
-- +------------------------------------------------------------------------------------------------------+

   CREATE DATABASE IF NOT EXISTS INSTRUCTOR2_DB1 DATA_RETENTION_TIME_IN_DAYS = 1;
USE DATABASE INSTRUCTOR2_DB1;
...
-- +------------------------------------------------------------------------------------------------------+
-- | Try to use role INSTRUCTOR2_DB1_SECADMIN                                                             |
-- +------------------------------------------------------------------------------------------------------+

USE ROLE INSTRUCTOR2_DB1_SECADMIN;

-- +------------------------------------------------------------------------------------------------------+
-- | Create Account Level functional roles                                                                |
-- +------------------------------------------------------------------------------------------------------+

   CREATE ROLE IF NOT EXISTS INSTRUCTOR2_DB1_ELT_SVC;
   CREATE ROLE IF NOT EXISTS INSTRUCTOR2_DB1_ANALYST;
   CREATE ROLE IF NOT EXISTS INSTRUCTOR2_DB1_DATA_SCI;
...
USE ROLE INSTRUCTOR2_DB1_SECADMIN;
   CREATE  ROLE IF NOT EXISTS _INSTRUCTOR2_DB1_ELT_WH_WU;
   CREATE  ROLE IF NOT EXISTS _INSTRUCTOR2_DB1_ELT_WH_WFULL;
   CREATE  ROLE IF NOT EXISTS _INSTRUCTOR2_DB1_ELT_WH_ALL;
   CREATE  ROLE IF NOT EXISTS _INSTRUCTOR2_DB1_ANALYST_WH_WU;
   CREATE  ROLE IF NOT EXISTS _INSTRUCTOR2_DB1_ANALYST_WH_WFULL;
   CREATE  ROLE IF NOT EXISTS _INSTRUCTOR2_DB1_ANALYST_WH_ALL;
   CREATE  ROLE IF NOT EXISTS _INSTRUCTOR2_DB1_DATA_SCI_WH_WU;
   CREATE  ROLE IF NOT EXISTS _INSTRUCTOR2_DB1_DATA_SCI_WH_WFULL;
   CREATE  ROLE IF NOT EXISTS _INSTRUCTOR2_DB1_DATA_SCI_WH_ALL;

-- +------------------------------------------------------------------------------------------------------+
-- | Grant ownership on warehouse access roles to INSTRUCTOR2_DB1_SECADMIN                                |
-- +------------------------------------------------------------------------------------------------------+

   GRANT OWNERSHIP ON ROLE _INSTRUCTOR2_DB1_ELT_WH_WU TO ROLE INSTRUCTOR2_DB1_SECADMIN;
   GRANT OWNERSHIP ON ROLE _INSTRUCTOR2_DB1_ELT_WH_WFULL TO ROLE INSTRUCTOR2_DB1_SECADMIN;
   GRANT OWNERSHIP ON ROLE _INSTRUCTOR2_DB1_ELT_WH_ALL TO ROLE INSTRUCTOR2_DB1_SECADMIN;
   GRANT OWNERSHIP ON ROLE _INSTRUCTOR2_DB1_ANALYST_WH_WU TO ROLE INSTRUCTOR2_DB1_SECADMIN;
   GRANT OWNERSHIP ON ROLE _INSTRUCTOR2_DB1_ANALYST_WH_WFULL TO ROLE INSTRUCTOR2_DB1_SECADMIN;
   GRANT OWNERSHIP ON ROLE _INSTRUCTOR2_DB1_ANALYST_WH_ALL TO ROLE INSTRUCTOR2_DB1_SECADMIN;
   GRANT OWNERSHIP ON ROLE _INSTRUCTOR2_DB1_DATA_SCI_WH_WU TO ROLE INSTRUCTOR2_DB1_SECADMIN;
   GRANT OWNERSHIP ON ROLE _INSTRUCTOR2_DB1_DATA_SCI_WH_WFULL TO ROLE INSTRUCTOR2_DB1_SECADMIN;
   GRANT OWNERSHIP ON ROLE _INSTRUCTOR2_DB1_DATA_SCI_WH_ALL TO ROLE INSTRUCTOR2_DB1_SECADMIN;
...
-- +------------------------------------------------------------------------------------------------------+
-- | Grant account level functional roles to INSTRUCTOR2_DB1_SYSADMIN                                     |
-- +------------------------------------------------------------------------------------------------------+

   GRANT ROLE INSTRUCTOR2_DB1_ELT_SVC TO ROLE INSTRUCTOR2_DB1_SYSADMIN;
   GRANT ROLE INSTRUCTOR2_DB1_ANALYST TO ROLE INSTRUCTOR2_DB1_SYSADMIN;
   GRANT ROLE INSTRUCTOR2_DB1_DATA_SCI TO ROLE INSTRUCTOR2_DB1_SYSADMIN;

-- +------------------------------------------------------------------------------------------------------+
-- | Grant access roles to INSTRUCTOR2_DB1_SYSADMIN                                                       |
-- +------------------------------------------------------------------------------------------------------+

USE ROLE INSTRUCTOR2_DB1_SYSADMIN;
   GRANT DATABASE ROLE _RAW_RO TO ROLE INSTRUCTOR2_DB1_SYSADMIN;
   GRANT DATABASE ROLE _RAW_RW TO ROLE INSTRUCTOR2_DB1_SYSADMIN;
   GRANT DATABASE ROLE _RAW_SFULL TO ROLE INSTRUCTOR2_DB1_SYSADMIN;
   GRANT DATABASE ROLE _EDW_RO TO ROLE INSTRUCTOR2_DB1_SYSADMIN;
   GRANT DATABASE ROLE _EDW_RW TO ROLE INSTRUCTOR2_DB1_SYSADMIN;
   GRANT DATABASE ROLE _EDW_SFULL TO ROLE INSTRUCTOR2_DB1_SYSADMIN;
   GRANT DATABASE ROLE _MART_RO TO ROLE INSTRUCTOR2_DB1_SYSADMIN;
   GRANT DATABASE ROLE _MART_RW TO ROLE INSTRUCTOR2_DB1_SYSADMIN;
   GRANT DATABASE ROLE _MART_SFULL TO ROLE INSTRUCTOR2_DB1_SYSADMIN;

-- +------------------------------------------------------------------------------------------------------+
-- | Grant access roles to functional roles                                                               |
-- +------------------------------------------------------------------------------------------------------+

   GRANT DATABASE ROLE _RAW_RW to role INSTRUCTOR2_DB1_ELT_SVC;
   GRANT DATABASE ROLE _RAW_RW to role INSTRUCTOR2_DB1_DATA_SCI;
   GRANT DATABASE ROLE _EDW_RW to role INSTRUCTOR2_DB1_ELT_SVC;
   GRANT DATABASE ROLE _EDW_RO to role INSTRUCTOR2_DB1_ANALYST;
   GRANT DATABASE ROLE _EDW_RO to role INSTRUCTOR2_DB1_DATA_SCI;
   GRANT DATABASE ROLE _MART_RW to role INSTRUCTOR2_DB1_ELT_SVC;
   GRANT DATABASE ROLE _MART_RO to role INSTRUCTOR2_DB1_ANALYST;
   GRANT DATABASE ROLE _MART_RO to role INSTRUCTOR2_DB1_DATA_SCI;

-- +------------------------------------------------------------------------------------------------------+
-- | Grant warehouse access roles to INSTRUCTOR2_DB1_SYSADMIN                                             |
-- +------------------------------------------------------------------------------------------------------+

USE ROLE INSTRUCTOR2_DB1_SECADMIN;
   GRANT ROLE _INSTRUCTOR2_DB1_ELT_WH_WU TO ROLE INSTRUCTOR2_DB1_SYSADMIN;
   GRANT ROLE _INSTRUCTOR2_DB1_ELT_WH_WFULL TO ROLE INSTRUCTOR2_DB1_SYSADMIN;
   GRANT ROLE _INSTRUCTOR2_DB1_ELT_WH_ALL TO ROLE INSTRUCTOR2_DB1_SYSADMIN;
   GRANT ROLE _INSTRUCTOR2_DB1_ANALYST_WH_WU TO ROLE INSTRUCTOR2_DB1_SYSADMIN;
   GRANT ROLE _INSTRUCTOR2_DB1_ANALYST_WH_WFULL TO ROLE INSTRUCTOR2_DB1_SYSADMIN;
   GRANT ROLE _INSTRUCTOR2_DB1_ANALYST_WH_ALL TO ROLE INSTRUCTOR2_DB1_SYSADMIN;
   GRANT ROLE _INSTRUCTOR2_DB1_DATA_SCI_WH_WU TO ROLE INSTRUCTOR2_DB1_SYSADMIN;
   GRANT ROLE _INSTRUCTOR2_DB1_DATA_SCI_WH_WFULL TO ROLE INSTRUCTOR2_DB1_SYSADMIN;
   GRANT ROLE _INSTRUCTOR2_DB1_DATA_SCI_WH_ALL TO ROLE INSTRUCTOR2_DB1_SYSADMIN;

-- +------------------------------------------------------------------------------------------------------+
-- | Grant virtual warehouse access roles to functional roles                                             |
-- +------------------------------------------------------------------------------------------------------+

   GRANT ROLE _INSTRUCTOR2_DB1_ELT_WH_WU TO ROLE INSTRUCTOR2_DB1_ELT_SVC;
   GRANT ROLE _INSTRUCTOR2_DB1_ANALYST_WH_WU TO ROLE INSTRUCTOR2_DB1_ANALYST;
   GRANT ROLE _INSTRUCTOR2_DB1_DATA_SCI_WH_WU TO ROLE INSTRUCTOR2_DB1_DATA_SCI;
...
-- +------------------------------------------------------------------------------------------------------+
   ...
   GRANT USAGE ON DATABASE INSTRUCTOR2_DB1 TO DATABASE ROLE _RAW_RO;
   GRANT USAGE ON DATABASE INSTRUCTOR2_DB1 TO DATABASE ROLE _RAW_RW;
   GRANT USAGE ON DATABASE INSTRUCTOR2_DB1 TO DATABASE ROLE _RAW_SFULL;
   GRANT USAGE ON DATABASE INSTRUCTOR2_DB1 TO DATABASE ROLE _EDW_RO;
   GRANT USAGE ON DATABASE INSTRUCTOR2_DB1 TO DATABASE ROLE _EDW_RW;
   GRANT USAGE ON DATABASE INSTRUCTOR2_DB1 TO DATABASE ROLE _EDW_SFULL;
   GRANT USAGE ON DATABASE INSTRUCTOR2_DB1 TO DATABASE ROLE _MART_RO;
   GRANT USAGE ON DATABASE INSTRUCTOR2_DB1 TO DATABASE ROLE _MART_RW;
   GRANT USAGE ON DATABASE INSTRUCTOR2_DB1 TO DATABASE ROLE _MART_SFULL;

-- +------------------------------------------------------------------------------------------------------+
-- | Create the warehouses                                                                                |
-- +------------------------------------------------------------------------------------------------------+

   CREATE WAREHOUSE IF NOT EXISTS INSTRUCTOR2_DB1_ELT_WH WITH WAREHOUSE_TYPE = 'STANDARD' warehouse_size='SMALL', scaling_policy='standard',min_cluster_count=1, max_cluster_count=1, auto_suspend=300,auto_resume=true, initially_suspended=true;
   CREATE WAREHOUSE IF NOT EXISTS INSTRUCTOR2_DB1_ANALYST_WH WITH WAREHOUSE_TYPE = 'STANDARD' warehouse_size='SMALL', scaling_policy='standard',min_cluster_count=1, max_cluster_count=1, auto_suspend=300,auto_resume=true, initially_suspended=true;
   CREATE WAREHOUSE IF NOT EXISTS INSTRUCTOR2_DB1_DATA_SCI_WH WITH WAREHOUSE_TYPE = 'STANDARD' warehouse_size='medium', scaling_policy='standard',min_cluster_count=1, max_cluster_count=1, auto_suspend=300,auto_resume=true, initially_suspended=true;

-- +------------------------------------------------------------------------------------------------------+
-- | Grant privileges on warehouses to access roles                                                       |
-- +------------------------------------------------------------------------------------------------------+

   GRANT USAGE ON WAREHOUSE INSTRUCTOR2_DB1_ELT_WH TO ROLE _INSTRUCTOR2_DB1_ELT_WH_WU;
   GRANT USAGE, OPERATE,MODIFY ON WAREHOUSE INSTRUCTOR2_DB1_ELT_WH TO ROLE _INSTRUCTOR2_DB1_ELT_WH_WFULL;
   GRANT ALL ON WAREHOUSE INSTRUCTOR2_DB1_ELT_WH TO ROLE _INSTRUCTOR2_DB1_ELT_WH_ALL;
   GRANT USAGE ON WAREHOUSE INSTRUCTOR2_DB1_ANALYST_WH TO ROLE _INSTRUCTOR2_DB1_ANALYST_WH_WU;
   GRANT USAGE, OPERATE,MODIFY ON WAREHOUSE INSTRUCTOR2_DB1_ANALYST_WH TO ROLE _INSTRUCTOR2_DB1_ANALYST_WH_WFULL;
   GRANT ALL ON WAREHOUSE INSTRUCTOR2_DB1_ANALYST_WH TO ROLE _INSTRUCTOR2_DB1_ANALYST_WH_ALL;
   GRANT USAGE ON WAREHOUSE INSTRUCTOR2_DB1_DATA_SCI_WH TO ROLE _INSTRUCTOR2_DB1_DATA_SCI_WH_WU;
   GRANT USAGE, OPERATE,MODIFY ON WAREHOUSE INSTRUCTOR2_DB1_DATA_SCI_WH TO ROLE _INSTRUCTOR2_DB1_DATA_SCI_WH_WFULL;
   GRANT ALL ON WAREHOUSE INSTRUCTOR2_DB1_DATA_SCI_WH TO ROLE _INSTRUCTOR2_DB1_DATA_SCI_WH_ALL;

-- +------------------------------------------------------------------------------------------------------+
-- | Create the schemas                                                                                   |
-- +------------------------------------------------------------------------------------------------------+

   CREATE SCHEMA IF NOT EXISTS INSTRUCTOR2_DB1.RAW WITH MANAGED ACCESS data_retention_time_in_days=1 comment = '';
   CREATE SCHEMA IF NOT EXISTS INSTRUCTOR2_DB1.EDW WITH MANAGED ACCESS data_retention_time_in_days=1 comment = '';
   CREATE SCHEMA IF NOT EXISTS INSTRUCTOR2_DB1.MART WITH MANAGED ACCESS data_retention_time_in_days=1 comment = '';
...
-- +------------------------------------------------------------------------------------------------------+
-- | Build of INSTRUCTOR2_DB1 complete - have a nice day                                                  |
-- +------------------------------------------------------------------------------------------------------+
