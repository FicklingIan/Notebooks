...
-- +------------------------------------------------------------------------------------------------------+
-- | Create the security roles for DB_GOV_SYSADMIN and DB_GOV_SECADMIN                                    |
-- +------------------------------------------------------------------------------------------------------+

   USE ROLE USERADMIN;

   CREATE ROLE IF NOT EXISTS DB_GOV_SECADMIN;
   CREATE ROLE IF NOT EXISTS DB_GOV_SYSADMIN;

   GRANT CREATE ROLE ON ACCOUNT TO DB_GOV_SECADMIN;
   GRANT ROLE DB_GOV_SECADMIN TO ROLE USERADMIN;
   GRANT ROLE DB_GOV_SYSADMIN TO ROLE SYSADMIN;

   USE ROLE SYSADMIN;

   GRANT CREATE DATABASE ON ACCOUNT TO DB_GOV_SYSADMIN;
   GRANT CREATE WAREHOUSE ON ACCOUNT TO DB_GOV_SYSADMIN;
...
-- +------------------------------------------------------------------------------------------------------+
-- | Try to use role DB_GOV_SYSADMIN                                                                      |
-- +------------------------------------------------------------------------------------------------------+

USE ROLE DB_GOV_SYSADMIN;

-- +------------------------------------------------------------------------------------------------------+
-- | Create database DB_GOV                                                                               |
-- +------------------------------------------------------------------------------------------------------+

   CREATE DATABASE IF NOT EXISTS DB_GOV DATA_RETENTION_TIME_IN_DAYS = 1;
USE DATABASE DB_GOV;
...
-- +------------------------------------------------------------------------------------------------------+
-- | Try to use role DB_GOV_SECADMIN                                                                      |
-- +------------------------------------------------------------------------------------------------------+

USE ROLE DB_GOV_SECADMIN;

-- +------------------------------------------------------------------------------------------------------+
-- | Create Account Level functional roles                                                                |
-- +------------------------------------------------------------------------------------------------------+

   CREATE ROLE IF NOT EXISTS DB_GOV_ETL_DEV;
   CREATE ROLE IF NOT EXISTS DB_GOV_ANALYST;
   CREATE ROLE IF NOT EXISTS DB_GOV_DATA_SCI;
   CREATE ROLE IF NOT EXISTS DB_GOV_POLICY_ADMIN;
...
USE ROLE DB_GOV_SECADMIN;
   CREATE  ROLE IF NOT EXISTS _DB_GOV_ADHOC_WH_WU;
   CREATE  ROLE IF NOT EXISTS _DB_GOV_ADHOC_WH_WFULL;
   CREATE  ROLE IF NOT EXISTS _DB_GOV_ADHOC_WH_ALL;

-- +------------------------------------------------------------------------------------------------------+
-- | Grant ownership on warehouse access roles to DB_GOV_SECADMIN                                         |
-- +------------------------------------------------------------------------------------------------------+

   GRANT OWNERSHIP ON ROLE _DB_GOV_ADHOC_WH_WU TO ROLE DB_GOV_SECADMIN;
   GRANT OWNERSHIP ON ROLE _DB_GOV_ADHOC_WH_WFULL TO ROLE DB_GOV_SECADMIN;
   GRANT OWNERSHIP ON ROLE _DB_GOV_ADHOC_WH_ALL TO ROLE DB_GOV_SECADMIN;
...
-- +------------------------------------------------------------------------------------------------------+
-- | Grant account level functional roles to DB_GOV_SYSADMIN                                              |
-- +------------------------------------------------------------------------------------------------------+

   GRANT ROLE DB_GOV_ETL_DEV TO ROLE DB_GOV_SYSADMIN;
   GRANT ROLE DB_GOV_ANALYST TO ROLE DB_GOV_SYSADMIN;
   GRANT ROLE DB_GOV_DATA_SCI TO ROLE DB_GOV_SYSADMIN;
   GRANT ROLE DB_GOV_POLICY_ADMIN TO ROLE DB_GOV_SYSADMIN;

-- +------------------------------------------------------------------------------------------------------+
-- | Grant access roles to DB_GOV_SYSADMIN                                                                |
-- +------------------------------------------------------------------------------------------------------+

   GRANT DATABASE ROLE _RAW_RO TO ROLE DB_GOV_SYSADMIN;
   GRANT DATABASE ROLE _RAW_RW TO ROLE DB_GOV_SYSADMIN;
   GRANT DATABASE ROLE _RAW_SFULL TO ROLE DB_GOV_SYSADMIN;
   GRANT DATABASE ROLE _EDW_RO TO ROLE DB_GOV_SYSADMIN;
   GRANT DATABASE ROLE _EDW_RW TO ROLE DB_GOV_SYSADMIN;
   GRANT DATABASE ROLE _EDW_SFULL TO ROLE DB_GOV_SYSADMIN;
   GRANT DATABASE ROLE _MART_RO TO ROLE DB_GOV_SYSADMIN;
   GRANT DATABASE ROLE _MART_RW TO ROLE DB_GOV_SYSADMIN;
   GRANT DATABASE ROLE _MART_SFULL TO ROLE DB_GOV_SYSADMIN;
   GRANT DATABASE ROLE _GOVERN_RO TO ROLE DB_GOV_SYSADMIN;
   GRANT DATABASE ROLE _GOVERN_RW TO ROLE DB_GOV_SYSADMIN;
   GRANT DATABASE ROLE _GOVERN_SFULL TO ROLE DB_GOV_SYSADMIN;

-- +------------------------------------------------------------------------------------------------------+
-- | Grant access roles to functional roles                                                               |
-- +------------------------------------------------------------------------------------------------------+

   GRANT DATABASE ROLE _RAW_SFULL to role DB_GOV_ETL_DEV;
   GRANT DATABASE ROLE _RAW_RO to role DB_GOV_ANALYST;
   GRANT DATABASE ROLE _RAW_RW to role DB_GOV_DATA_SCI;
   GRANT DATABASE ROLE _RAW_RO to role DB_GOV_POLICY_ADMIN;
   GRANT DATABASE ROLE _EDW_SFULL to role DB_GOV_ETL_DEV;
   GRANT DATABASE ROLE _EDW_RO to role DB_GOV_ANALYST;
   GRANT DATABASE ROLE _EDW_RO to role DB_GOV_DATA_SCI;
   GRANT DATABASE ROLE _EDW_RO to role DB_GOV_POLICY_ADMIN;
   GRANT DATABASE ROLE _MART_SFULL to role DB_GOV_ETL_DEV;
   GRANT DATABASE ROLE _MART_RO to role DB_GOV_ANALYST;
   GRANT DATABASE ROLE _MART_RO to role DB_GOV_DATA_SCI;
   GRANT DATABASE ROLE _MART_RO to role DB_GOV_POLICY_ADMIN;
   GRANT DATABASE ROLE _GOVERN_SFULL to role DB_GOV_ETL_DEV;
   GRANT DATABASE ROLE _GOVERN_RO to role DB_GOV_ANALYST;
   GRANT DATABASE ROLE _GOVERN_RO to role DB_GOV_DATA_SCI;
   GRANT DATABASE ROLE _GOVERN_RW to role DB_GOV_POLICY_ADMIN;

-- +------------------------------------------------------------------------------------------------------+
-- | Grant warehouse access roles to DB_GOV_SYSADMIN                                                      |
-- +------------------------------------------------------------------------------------------------------+

USE ROLE DB_GOV_SECADMIN;
   GRANT ROLE _DB_GOV_ADHOC_WH_WU TO ROLE DB_GOV_SYSADMIN;
   GRANT ROLE _DB_GOV_ADHOC_WH_WFULL TO ROLE DB_GOV_SYSADMIN;
   GRANT ROLE _DB_GOV_ADHOC_WH_ALL TO ROLE DB_GOV_SYSADMIN;

-- +------------------------------------------------------------------------------------------------------+
-- | Grant virtual warehouse access roles to functional roles                                             |
-- +------------------------------------------------------------------------------------------------------+

   GRANT ROLE _DB_GOV_ADHOC_WH_WU TO ROLE DB_GOV_ETL_DEV;
   GRANT ROLE _DB_GOV_ADHOC_WH_WU TO ROLE DB_GOV_ANALYST;
   GRANT ROLE _DB_GOV_ADHOC_WH_WU TO ROLE DB_GOV_DATA_SCI;
   GRANT ROLE _DB_GOV_ADHOC_WH_WU TO ROLE DB_GOV_POLICY_ADMIN;
...
-- +------------------------------------------------------------------------------------------------------+
   ...
   GRANT USAGE ON DATABASE DB_GOV TO DATABASE ROLE _RAW_RO;
   GRANT USAGE ON DATABASE DB_GOV TO DATABASE ROLE _RAW_RW;
   GRANT USAGE ON DATABASE DB_GOV TO DATABASE ROLE _RAW_SFULL;
   GRANT USAGE ON DATABASE DB_GOV TO DATABASE ROLE _EDW_RO;
   GRANT USAGE ON DATABASE DB_GOV TO DATABASE ROLE _EDW_RW;
   GRANT USAGE ON DATABASE DB_GOV TO DATABASE ROLE _EDW_SFULL;
   GRANT USAGE ON DATABASE DB_GOV TO DATABASE ROLE _MART_RO;
   GRANT USAGE ON DATABASE DB_GOV TO DATABASE ROLE _MART_RW;
   GRANT USAGE ON DATABASE DB_GOV TO DATABASE ROLE _MART_SFULL;
   GRANT USAGE ON DATABASE DB_GOV TO DATABASE ROLE _GOVERN_RO;
   GRANT USAGE ON DATABASE DB_GOV TO DATABASE ROLE _GOVERN_RW;
   GRANT USAGE ON DATABASE DB_GOV TO DATABASE ROLE _GOVERN_SFULL;

-- +------------------------------------------------------------------------------------------------------+
-- | Create the warehouses                                                                                |
-- +------------------------------------------------------------------------------------------------------+

   CREATE WAREHOUSE IF NOT EXISTS DB_GOV_ADHOC_WH WITH WAREHOUSE_TYPE = 'STANDARD' warehouse_size='xsmall', scaling_policy='standard',min_cluster_count=1, max_cluster_count=1, auto_suspend=300,auto_resume=true, initially_suspended=true;

-- +------------------------------------------------------------------------------------------------------+
-- | Grant privileges on warehouses to access roles                                                       |
-- +------------------------------------------------------------------------------------------------------+

   GRANT USAGE ON WAREHOUSE DB_GOV_ADHOC_WH TO ROLE _DB_GOV_ADHOC_WH_WU;
   GRANT USAGE, OPERATE,MODIFY ON WAREHOUSE DB_GOV_ADHOC_WH TO ROLE _DB_GOV_ADHOC_WH_WFULL;
   GRANT ALL ON WAREHOUSE DB_GOV_ADHOC_WH TO ROLE _DB_GOV_ADHOC_WH_ALL;

-- +------------------------------------------------------------------------------------------------------+
-- | Create the schemas                                                                                   |
-- +------------------------------------------------------------------------------------------------------+

   CREATE SCHEMA IF NOT EXISTS DB_GOV.RAW WITH MANAGED ACCESS data_retention_time_in_days=1 comment = '';
   CREATE SCHEMA IF NOT EXISTS DB_GOV.EDW WITH MANAGED ACCESS data_retention_time_in_days=1 comment = '';
   CREATE SCHEMA IF NOT EXISTS DB_GOV.MART WITH MANAGED ACCESS data_retention_time_in_days=1 comment = '';
   CREATE SCHEMA IF NOT EXISTS DB_GOV.GOVERN WITH MANAGED ACCESS data_retention_time_in_days=1 comment = '';
...
-- +------------------------------------------------------------------------------------------------------+
-- | Build of DB_GOV complete - have a nice day                                                           |
-- +------------------------------------------------------------------------------------------------------+
