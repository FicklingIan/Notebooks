...
-- +------------------------------------------------------------------------------------------------------+
-- | Create the security roles for PROD_ARCH_SYSADMIN and PROD_ARCH_SECADMIN                              |
-- +------------------------------------------------------------------------------------------------------+

   USE ROLE USERADMIN;

   CREATE ROLE IF NOT EXISTS PROD_ARCH_SECADMIN;
   CREATE ROLE IF NOT EXISTS PROD_ARCH_SYSADMIN;

   GRANT CREATE ROLE ON ACCOUNT TO PROD_ARCH_SECADMIN;
   GRANT ROLE PROD_ARCH_SECADMIN TO ROLE USERADMIN;
   GRANT ROLE PROD_ARCH_SYSADMIN TO ROLE SYSADMIN;

   USE ROLE SYSADMIN;

   GRANT CREATE DATABASE ON ACCOUNT TO PROD_ARCH_SYSADMIN;
   GRANT CREATE WAREHOUSE ON ACCOUNT TO PROD_ARCH_SYSADMIN;
...
-- +------------------------------------------------------------------------------------------------------+
-- | Try to use role PROD_ARCH_SYSADMIN                                                                   |
-- +------------------------------------------------------------------------------------------------------+

USE ROLE PROD_ARCH_SYSADMIN;

-- +------------------------------------------------------------------------------------------------------+
-- | Create database PROD_ARCH                                                                            |
-- +------------------------------------------------------------------------------------------------------+

   CREATE DATABASE IF NOT EXISTS PROD_ARCH DATA_RETENTION_TIME_IN_DAYS = 1;
USE DATABASE PROD_ARCH;
...
-- +------------------------------------------------------------------------------------------------------+
-- | Try to use role PROD_ARCH_SECADMIN                                                                   |
-- +------------------------------------------------------------------------------------------------------+

USE ROLE PROD_ARCH_SECADMIN;

-- +------------------------------------------------------------------------------------------------------+
-- | Create Account Level functional roles                                                                |
-- +------------------------------------------------------------------------------------------------------+

   CREATE ROLE IF NOT EXISTS PROD_ARCH_ELT_DEV;
   CREATE ROLE IF NOT EXISTS PROD_ARCH_ANALYST;
   CREATE ROLE IF NOT EXISTS PROD_ARCH_DATA_SCI;
...
USE ROLE PROD_ARCH_SECADMIN;
   CREATE  ROLE IF NOT EXISTS _PROD_ARCH_ADHOC_WH_WU;
   CREATE  ROLE IF NOT EXISTS _PROD_ARCH_ADHOC_WH_WFULL;
   CREATE  ROLE IF NOT EXISTS _PROD_ARCH_ADHOC_WH_ALL;

-- +------------------------------------------------------------------------------------------------------+
-- | Grant ownership on warehouse access roles to PROD_ARCH_SECADMIN                                      |
-- +------------------------------------------------------------------------------------------------------+

   GRANT OWNERSHIP ON ROLE _PROD_ARCH_ADHOC_WH_WU TO ROLE PROD_ARCH_SECADMIN;
   GRANT OWNERSHIP ON ROLE _PROD_ARCH_ADHOC_WH_WFULL TO ROLE PROD_ARCH_SECADMIN;
   GRANT OWNERSHIP ON ROLE _PROD_ARCH_ADHOC_WH_ALL TO ROLE PROD_ARCH_SECADMIN;
...
-- +------------------------------------------------------------------------------------------------------+
-- | Grant account level functional roles to PROD_ARCH_SYSADMIN                                           |
-- +------------------------------------------------------------------------------------------------------+

   GRANT ROLE PROD_ARCH_ELT_DEV TO ROLE PROD_ARCH_SYSADMIN;
   GRANT ROLE PROD_ARCH_ANALYST TO ROLE PROD_ARCH_SYSADMIN;
   GRANT ROLE PROD_ARCH_DATA_SCI TO ROLE PROD_ARCH_SYSADMIN;

-- +------------------------------------------------------------------------------------------------------+
-- | Grant access roles to PROD_ARCH_SYSADMIN                                                             |
-- +------------------------------------------------------------------------------------------------------+

   GRANT DATABASE ROLE _RAW_RO TO ROLE PROD_ARCH_SYSADMIN;
   GRANT DATABASE ROLE _RAW_RW TO ROLE PROD_ARCH_SYSADMIN;
   GRANT DATABASE ROLE _RAW_SFULL TO ROLE PROD_ARCH_SYSADMIN;
   GRANT DATABASE ROLE _EDW_RO TO ROLE PROD_ARCH_SYSADMIN;
   GRANT DATABASE ROLE _EDW_RW TO ROLE PROD_ARCH_SYSADMIN;
   GRANT DATABASE ROLE _EDW_SFULL TO ROLE PROD_ARCH_SYSADMIN;
   GRANT DATABASE ROLE _MART_RO TO ROLE PROD_ARCH_SYSADMIN;
   GRANT DATABASE ROLE _MART_RW TO ROLE PROD_ARCH_SYSADMIN;
   GRANT DATABASE ROLE _MART_SFULL TO ROLE PROD_ARCH_SYSADMIN;

-- +------------------------------------------------------------------------------------------------------+
-- | Grant access roles to functional roles                                                               |
-- +------------------------------------------------------------------------------------------------------+

   GRANT DATABASE ROLE _RAW_SFULL to role PROD_ARCH_ELT_DEV;
   GRANT DATABASE ROLE _RAW_RO to role PROD_ARCH_ANALYST;
   GRANT DATABASE ROLE _RAW_RW to role PROD_ARCH_DATA_SCI;
   GRANT DATABASE ROLE _EDW_SFULL to role PROD_ARCH_ELT_DEV;
   GRANT DATABASE ROLE _EDW_RO to role PROD_ARCH_ANALYST;
   GRANT DATABASE ROLE _EDW_RO to role PROD_ARCH_DATA_SCI;
   GRANT DATABASE ROLE _MART_SFULL to role PROD_ARCH_ELT_DEV;
   GRANT DATABASE ROLE _MART_RO to role PROD_ARCH_ANALYST;
   GRANT DATABASE ROLE _MART_RO to role PROD_ARCH_DATA_SCI;

-- +------------------------------------------------------------------------------------------------------+
-- | Grant warehouse access roles to PROD_ARCH_SYSADMIN                                                   |
-- +------------------------------------------------------------------------------------------------------+

USE ROLE PROD_ARCH_SECADMIN;
   GRANT ROLE _PROD_ARCH_ADHOC_WH_WU TO ROLE PROD_ARCH_SYSADMIN;
   GRANT ROLE _PROD_ARCH_ADHOC_WH_WFULL TO ROLE PROD_ARCH_SYSADMIN;
   GRANT ROLE _PROD_ARCH_ADHOC_WH_ALL TO ROLE PROD_ARCH_SYSADMIN;

-- +------------------------------------------------------------------------------------------------------+
-- | Grant virtual warehouse access roles to functional roles                                             |
-- +------------------------------------------------------------------------------------------------------+

   GRANT ROLE _PROD_ARCH_ADHOC_WH_WU TO ROLE PROD_ARCH_ELT_DEV;
   GRANT ROLE _PROD_ARCH_ADHOC_WH_WU TO ROLE PROD_ARCH_ANALYST;
   GRANT ROLE _PROD_ARCH_ADHOC_WH_WU TO ROLE PROD_ARCH_DATA_SCI;
...
-- +------------------------------------------------------------------------------------------------------+
   ...
   GRANT USAGE ON DATABASE PROD_ARCH TO DATABASE ROLE _RAW_RO;
   GRANT USAGE ON DATABASE PROD_ARCH TO DATABASE ROLE _RAW_RW;
   GRANT USAGE ON DATABASE PROD_ARCH TO DATABASE ROLE _RAW_SFULL;
   GRANT USAGE ON DATABASE PROD_ARCH TO DATABASE ROLE _EDW_RO;
   GRANT USAGE ON DATABASE PROD_ARCH TO DATABASE ROLE _EDW_RW;
   GRANT USAGE ON DATABASE PROD_ARCH TO DATABASE ROLE _EDW_SFULL;
   GRANT USAGE ON DATABASE PROD_ARCH TO DATABASE ROLE _MART_RO;
   GRANT USAGE ON DATABASE PROD_ARCH TO DATABASE ROLE _MART_RW;
   GRANT USAGE ON DATABASE PROD_ARCH TO DATABASE ROLE _MART_SFULL;

-- +------------------------------------------------------------------------------------------------------+
-- | Create the warehouses                                                                                |
-- +------------------------------------------------------------------------------------------------------+

   CREATE WAREHOUSE IF NOT EXISTS PROD_ARCH_ADHOC_WH WITH WAREHOUSE_TYPE = 'STANDARD' warehouse_size='xsmall', scaling_policy='standard',min_cluster_count=1, max_cluster_count=1, auto_suspend=300,auto_resume=true, initially_suspended=true;

-- +------------------------------------------------------------------------------------------------------+
-- | Grant privileges on warehouses to access roles                                                       |
-- +------------------------------------------------------------------------------------------------------+

   GRANT USAGE ON WAREHOUSE PROD_ARCH_ADHOC_WH TO ROLE _PROD_ARCH_ADHOC_WH_WU;
   GRANT USAGE, OPERATE,MODIFY ON WAREHOUSE PROD_ARCH_ADHOC_WH TO ROLE _PROD_ARCH_ADHOC_WH_WFULL;
   GRANT ALL ON WAREHOUSE PROD_ARCH_ADHOC_WH TO ROLE _PROD_ARCH_ADHOC_WH_ALL;

-- +------------------------------------------------------------------------------------------------------+
-- | Create the schemas                                                                                   |
-- +------------------------------------------------------------------------------------------------------+

   CREATE SCHEMA IF NOT EXISTS PROD_ARCH.RAW WITH MANAGED ACCESS data_retention_time_in_days=1 comment = '';
   CREATE SCHEMA IF NOT EXISTS PROD_ARCH.EDW WITH MANAGED ACCESS data_retention_time_in_days=1 comment = '';
   CREATE SCHEMA IF NOT EXISTS PROD_ARCH.MART WITH MANAGED ACCESS data_retention_time_in_days=1 comment = '';
...
-- +------------------------------------------------------------------------------------------------------+
-- | Build of PROD_ARCH complete - have a nice day                                                        |
-- +------------------------------------------------------------------------------------------------------+
