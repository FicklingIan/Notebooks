...
-- * Date           : 2023-11-06                    *
-- * Time           : 14:46                         *
-- * Spreadsheet    : SpreadSheet/EDU_Examples.xlsx *
-- * Environment(s) : PROD_EDW                      *
-- **************************************************


-- +------------------------------------------------------------------------------------------------------+
-- | Create the security roles for PROD_EDW2_SYSADMIN and PROD_EDW2_SECADMIN                              |
-- +------------------------------------------------------------------------------------------------------+

   USE ROLE USERADMIN;

   CREATE ROLE IF NOT EXISTS PROD_EDW2_SECADMIN;
   CREATE ROLE IF NOT EXISTS PROD_EDW2_SYSADMIN;

   GRANT CREATE ROLE ON ACCOUNT TO PROD_EDW2_SECADMIN;
   GRANT ROLE PROD_EDW2_SECADMIN TO ROLE USERADMIN;
   GRANT ROLE PROD_EDW2_SYSADMIN TO ROLE SYSADMIN;

   USE ROLE SYSADMIN;

   GRANT CREATE DATABASE ON ACCOUNT TO PROD_EDW2_SYSADMIN;
   GRANT CREATE WAREHOUSE ON ACCOUNT TO PROD_EDW2_SYSADMIN;
...
-- +------------------------------------------------------------------------------------------------------+
-- | Try to use role PROD_EDW2_SYSADMIN                                                                   |
-- +------------------------------------------------------------------------------------------------------+

USE ROLE PROD_EDW2_SYSADMIN;

-- +------------------------------------------------------------------------------------------------------+
-- | Create database PROD_EDW2                                                                            |
-- +------------------------------------------------------------------------------------------------------+

   CREATE DATABASE IF NOT EXISTS PROD_EDW2 DATA_RETENTION_TIME_IN_DAYS = 1;
USE DATABASE PROD_EDW2;
...
-- +------------------------------------------------------------------------------------------------------+
-- | Try to use role PROD_EDW2_SECADMIN                                                                   |
-- +------------------------------------------------------------------------------------------------------+

USE ROLE PROD_EDW2_SECADMIN;

-- +------------------------------------------------------------------------------------------------------+
-- | Create Account Level functional roles                                                                |
-- +------------------------------------------------------------------------------------------------------+

   CREATE ROLE IF NOT EXISTS PROD_EDW2_ETL_DEV;
   CREATE ROLE IF NOT EXISTS PROD_EDW2_ANALYST;
   CREATE ROLE IF NOT EXISTS PROD_EDW2_DATA_SCI;
   CREATE ROLE IF NOT EXISTS PROD_EDW2_POLICY_ADMIN;
...
USE ROLE PROD_EDW2_SECADMIN;
   CREATE  ROLE IF NOT EXISTS _PROD_EDW2_ADHOC_WH_WU;
   CREATE  ROLE IF NOT EXISTS _PROD_EDW2_ADHOC_WH_WFULL;
   CREATE  ROLE IF NOT EXISTS _PROD_EDW2_ADHOC_WH_ALL;

-- +------------------------------------------------------------------------------------------------------+
-- | Grant ownership on warehouse access roles to PROD_EDW2_SECADMIN                                      |
-- +------------------------------------------------------------------------------------------------------+

   GRANT OWNERSHIP ON ROLE _PROD_EDW2_ADHOC_WH_WU TO ROLE PROD_EDW2_SECADMIN;
   GRANT OWNERSHIP ON ROLE _PROD_EDW2_ADHOC_WH_WFULL TO ROLE PROD_EDW2_SECADMIN;
   GRANT OWNERSHIP ON ROLE _PROD_EDW2_ADHOC_WH_ALL TO ROLE PROD_EDW2_SECADMIN;

-- +------------------------------------------------------------------------------------------------------+
-- | Create Super-Roles  (ownership stays with USERADMIN)                                                 |
-- +------------------------------------------------------------------------------------------------------+

   CREATE ROLE IF NOT EXISTS SUPER_ELT;
...
-- +------------------------------------------------------------------------------------------------------+
   ...
   GRANT ROLE SUPER_ELT TO USER SF_SUPER_USER;
   GRANT ROLE SUPER_READER TO USER SF_SUPER_USER;
   GRANT ROLE SUPER_WRITER TO USER SF_SUPER_USER;
...
-- +------------------------------------------------------------------------------------------------------+
-- | Grant account level functional roles to PROD_EDW2_SYSADMIN                                           |
-- +------------------------------------------------------------------------------------------------------+

   GRANT ROLE PROD_EDW2_ETL_DEV TO ROLE PROD_EDW2_SYSADMIN;
   GRANT ROLE PROD_EDW2_ANALYST TO ROLE PROD_EDW2_SYSADMIN;
   GRANT ROLE PROD_EDW2_DATA_SCI TO ROLE PROD_EDW2_SYSADMIN;
   GRANT ROLE PROD_EDW2_POLICY_ADMIN TO ROLE PROD_EDW2_SYSADMIN;

-- +------------------------------------------------------------------------------------------------------+
-- | Grant database functional roles to PROD_EDW2_SYSADMIN                                                |
-- +------------------------------------------------------------------------------------------------------+

USE ROLE PROD_EDW2_SYSADMIN;
USE DATABASE PROD_EDW2;
   GRANT DATABASE ROLE ETL_DEV TO ROLE PROD_EDW2_SYSADMIN;
   GRANT DATABASE ROLE ANALYST TO ROLE PROD_EDW2_SYSADMIN;
   GRANT DATABASE ROLE DATA_SCI TO ROLE PROD_EDW2_SYSADMIN;
   GRANT DATABASE ROLE POLICY_ADMIN TO ROLE PROD_EDW2_SYSADMIN;

-- +------------------------------------------------------------------------------------------------------+
-- | Grant database functional roles to account level functional roles                                    |
-- +------------------------------------------------------------------------------------------------------+

   GRANT DATABASE ROLE ETL_DEV TO ROLE PROD_EDW2_ETL_DEV;
   GRANT DATABASE ROLE ANALYST TO ROLE PROD_EDW2_ANALYST;
   GRANT DATABASE ROLE DATA_SCI TO ROLE PROD_EDW2_DATA_SCI;
   GRANT DATABASE ROLE POLICY_ADMIN TO ROLE PROD_EDW2_POLICY_ADMIN;

-- +------------------------------------------------------------------------------------------------------+
-- | Grant access roles to PROD_EDW2_SYSADMIN                                                             |
-- +------------------------------------------------------------------------------------------------------+

   GRANT DATABASE ROLE _RAW_RO TO ROLE PROD_EDW2_SYSADMIN;
   GRANT DATABASE ROLE _RAW_SU TO ROLE PROD_EDW2_SYSADMIN;
   GRANT DATABASE ROLE _RAW_RW TO ROLE PROD_EDW2_SYSADMIN;
   GRANT DATABASE ROLE _RAW_SFULL TO ROLE PROD_EDW2_SYSADMIN;
   GRANT DATABASE ROLE _EDW_RO TO ROLE PROD_EDW2_SYSADMIN;
   GRANT DATABASE ROLE _EDW_SU TO ROLE PROD_EDW2_SYSADMIN;
   GRANT DATABASE ROLE _EDW_RW TO ROLE PROD_EDW2_SYSADMIN;
   GRANT DATABASE ROLE _EDW_SFULL TO ROLE PROD_EDW2_SYSADMIN;
   GRANT DATABASE ROLE _MART_RO TO ROLE PROD_EDW2_SYSADMIN;
   GRANT DATABASE ROLE _MART_SU TO ROLE PROD_EDW2_SYSADMIN;
   GRANT DATABASE ROLE _MART_RW TO ROLE PROD_EDW2_SYSADMIN;
   GRANT DATABASE ROLE _MART_SFULL TO ROLE PROD_EDW2_SYSADMIN;
   GRANT DATABASE ROLE _GOVERN_RO TO ROLE PROD_EDW2_SYSADMIN;
   GRANT DATABASE ROLE _GOVERN_SU TO ROLE PROD_EDW2_SYSADMIN;
   GRANT DATABASE ROLE _GOVERN_RW TO ROLE PROD_EDW2_SYSADMIN;
   GRANT DATABASE ROLE _GOVERN_SFULL TO ROLE PROD_EDW2_SYSADMIN;
...
-- +------------------------------------------------------------------------------------------------------+
   ...
   GRANT DATABASE ROLE _RAW_SFULL TO ROLE PROD_EDW2_ETL_DEV;
   GRANT DATABASE ROLE _RAW_RO TO DATABASE ROLE ANALYST;
   GRANT DATABASE ROLE _RAW_RO TO ROLE PROD_EDW2_ANALYST;
   GRANT DATABASE ROLE _RAW_RW TO DATABASE ROLE DATA_SCI;
   GRANT DATABASE ROLE _RAW_RW TO ROLE PROD_EDW2_DATA_SCI;
   GRANT DATABASE ROLE _EDW_SFULL TO DATABASE ROLE ETL_DEV;
   GRANT DATABASE ROLE _EDW_SFULL TO ROLE PROD_EDW2_ETL_DEV;
   GRANT DATABASE ROLE _EDW_RO TO DATABASE ROLE ANALYST;
   GRANT DATABASE ROLE _EDW_RO TO ROLE PROD_EDW2_ANALYST;
   GRANT DATABASE ROLE _EDW_RO TO DATABASE ROLE DATA_SCI;
   GRANT DATABASE ROLE _EDW_RO TO ROLE PROD_EDW2_DATA_SCI;
   GRANT DATABASE ROLE _EDW_RO TO DATABASE ROLE POLICY_ADMIN;
   GRANT DATABASE ROLE _EDW_RO TO ROLE PROD_EDW2_POLICY_ADMIN;
   GRANT DATABASE ROLE _MART_SFULL TO DATABASE ROLE ETL_DEV;
   GRANT DATABASE ROLE _MART_SFULL TO ROLE PROD_EDW2_ETL_DEV;
   GRANT DATABASE ROLE _MART_RO TO DATABASE ROLE ANALYST;
   GRANT DATABASE ROLE _MART_RO TO ROLE PROD_EDW2_ANALYST;
   GRANT DATABASE ROLE _MART_RO TO DATABASE ROLE DATA_SCI;
   GRANT DATABASE ROLE _MART_RO TO ROLE PROD_EDW2_DATA_SCI;
   GRANT DATABASE ROLE _MART_RO TO DATABASE ROLE POLICY_ADMIN;
   GRANT DATABASE ROLE _MART_RO TO ROLE PROD_EDW2_POLICY_ADMIN;
   GRANT DATABASE ROLE _GOVERN_SFULL TO DATABASE ROLE POLICY_ADMIN;
   GRANT DATABASE ROLE _GOVERN_SFULL TO ROLE PROD_EDW2_POLICY_ADMIN;

-- +------------------------------------------------------------------------------------------------------+
-- | Grant warehouse access roles to PROD_EDW2_SYSADMIN                                                   |
-- +------------------------------------------------------------------------------------------------------+

USE ROLE PROD_EDW2_SECADMIN;
   GRANT ROLE _PROD_EDW2_ADHOC_WH_WU TO ROLE PROD_EDW2_SYSADMIN;
   GRANT ROLE _PROD_EDW2_ADHOC_WH_WFULL TO ROLE PROD_EDW2_SYSADMIN;
   GRANT ROLE _PROD_EDW2_ADHOC_WH_ALL TO ROLE PROD_EDW2_SYSADMIN;

-- +------------------------------------------------------------------------------------------------------+
-- | Grant virtual warehouse access roles to functional roles                                             |
-- +------------------------------------------------------------------------------------------------------+

   GRANT ROLE _PROD_EDW2_ADHOC_WH_WU TO ROLE PROD_EDW2_ETL_DEV;
   GRANT ROLE _PROD_EDW2_ADHOC_WH_WU TO ROLE PROD_EDW2_ANALYST;
   GRANT ROLE _PROD_EDW2_ADHOC_WH_WU TO ROLE PROD_EDW2_DATA_SCI;
   GRANT ROLE _PROD_EDW2_ADHOC_WH_WU TO ROLE PROD_EDW2_POLICY_ADMIN;

-- +------------------------------------------------------------------------------------------------------+
-- | Grant functional roles to super-roles                                                                |
-- +------------------------------------------------------------------------------------------------------+

   GRANT ROLE PROD_EDW2_ETL_DEV TO ROLE SUPER_WRITER;
   GRANT ROLE PROD_EDW2_DATA_SCI TO ROLE SUPER_WRITER;
   GRANT ROLE PROD_EDW2_POLICY_ADMIN TO ROLE SUPER_WRITER;
   GRANT ROLE PROD_EDW2_ANALYST TO ROLE SUPER_READER;
   GRANT ROLE PROD_EDW2_DATA_SCI TO ROLE SUPER_READER;
   GRANT ROLE PROD_EDW2_POLICY_ADMIN TO ROLE SUPER_READER;
   GRANT ROLE PROD_EDW2_ETL_DEV TO ROLE SUPER_ELT;
   GRANT ROLE PROD_EDW2_POLICY_ADMIN TO ROLE SUPER_ELT;

-- +------------------------------------------------------------------------------------------------------+
-- | Grant functional roles to users                                                                      |
-- +------------------------------------------------------------------------------------------------------+

   GRANT ROLE PROD_EDW2_ETL_DEV TO USER SF_ELT_DEV;
   GRANT ROLE PROD_EDW2_ANALYST TO USER SF_ANALYST;
   GRANT ROLE PROD_EDW2_DATA_SCI TO USER SF_DATA_SCIENTIST;
   GRANT ROLE PROD_EDW2_POLICY_ADMIN TO USER SF_DATA_GOVERANCE;
...
-- +------------------------------------------------------------------------------------------------------+
   ...
   GRANT USAGE ON DATABASE PROD_EDW2 TO DATABASE ROLE _RAW_RO;
   GRANT USAGE ON DATABASE PROD_EDW2 TO DATABASE ROLE _RAW_SU;
   GRANT USAGE ON DATABASE PROD_EDW2 TO DATABASE ROLE _RAW_RW;
   GRANT USAGE ON DATABASE PROD_EDW2 TO DATABASE ROLE _RAW_SFULL;
   GRANT USAGE ON DATABASE PROD_EDW2 TO DATABASE ROLE _EDW_RO;
   GRANT USAGE ON DATABASE PROD_EDW2 TO DATABASE ROLE _EDW_SU;
   GRANT USAGE ON DATABASE PROD_EDW2 TO DATABASE ROLE _EDW_RW;
   GRANT USAGE ON DATABASE PROD_EDW2 TO DATABASE ROLE _EDW_SFULL;
   GRANT USAGE ON DATABASE PROD_EDW2 TO DATABASE ROLE _MART_RO;
   GRANT USAGE ON DATABASE PROD_EDW2 TO DATABASE ROLE _MART_SU;
   GRANT USAGE ON DATABASE PROD_EDW2 TO DATABASE ROLE _MART_RW;
   GRANT USAGE ON DATABASE PROD_EDW2 TO DATABASE ROLE _MART_SFULL;
   GRANT USAGE ON DATABASE PROD_EDW2 TO DATABASE ROLE _GOVERN_RO;
   GRANT USAGE ON DATABASE PROD_EDW2 TO DATABASE ROLE _GOVERN_SU;
   GRANT USAGE ON DATABASE PROD_EDW2 TO DATABASE ROLE _GOVERN_RW;
   GRANT USAGE ON DATABASE PROD_EDW2 TO DATABASE ROLE _GOVERN_SFULL;

-- +------------------------------------------------------------------------------------------------------+
-- | Create the warehouses                                                                                |
-- +------------------------------------------------------------------------------------------------------+

   CREATE WAREHOUSE IF NOT EXISTS PROD_EDW2_ADHOC_WH WITH WAREHOUSE_TYPE = 'STANDARD' warehouse_size='xsmall', scaling_policy='standard',min_cluster_count=1, max_cluster_count=1, auto_suspend=300,auto_resume=true, initially_suspended=true;

-- +------------------------------------------------------------------------------------------------------+
-- | Grant privileges on warehouses to access roles                                                       |
-- +------------------------------------------------------------------------------------------------------+

   GRANT USAGE ON WAREHOUSE PROD_EDW2_ADHOC_WH TO ROLE _PROD_EDW2_ADHOC_WH_WU;
   GRANT USAGE, OPERATE,MODIFY ON WAREHOUSE PROD_EDW2_ADHOC_WH TO ROLE _PROD_EDW2_ADHOC_WH_WFULL;
   GRANT ALL ON WAREHOUSE PROD_EDW2_ADHOC_WH TO ROLE _PROD_EDW2_ADHOC_WH_ALL;

-- +------------------------------------------------------------------------------------------------------+
-- | Create the schemas                                                                                   |
-- +------------------------------------------------------------------------------------------------------+

   CREATE SCHEMA IF NOT EXISTS PROD_EDW2.RAW WITH MANAGED ACCESS data_retention_time_in_days=1 comment = '';
   CREATE SCHEMA IF NOT EXISTS PROD_EDW2.EDW WITH MANAGED ACCESS data_retention_time_in_days=1 comment = '';
   CREATE SCHEMA IF NOT EXISTS PROD_EDW2.MART WITH MANAGED ACCESS data_retention_time_in_days=1 comment = '';
   CREATE SCHEMA IF NOT EXISTS PROD_EDW2.GOVERN WITH MANAGED ACCESS data_retention_time_in_days=1 comment = '';
...
-- +------------------------------------------------------------------------------------------------------+
-- | Build of PROD_EDW2 complete - have a nice day                                                        |
-- +------------------------------------------------------------------------------------------------------+
