...
-- +------------------------------------------------------------------------------------------------------+
-- | Create the security roles for INMSTRUCTOR1_DB_SYSADMIN and INMSTRUCTOR1_DB_SECADMIN                  |
-- +------------------------------------------------------------------------------------------------------+

   USE ROLE USERADMIN;

   CREATE ROLE IF NOT EXISTS INMSTRUCTOR1_DB_SECADMIN;
   CREATE ROLE IF NOT EXISTS INMSTRUCTOR1_DB_SYSADMIN;

   GRANT CREATE ROLE ON ACCOUNT TO INMSTRUCTOR1_DB_SECADMIN;
   GRANT ROLE INMSTRUCTOR1_DB_SECADMIN TO ROLE USERADMIN;
   GRANT ROLE INMSTRUCTOR1_DB_SYSADMIN TO ROLE SYSADMIN;

   USE ROLE SYSADMIN;

   GRANT CREATE DATABASE ON ACCOUNT TO INMSTRUCTOR1_DB_SYSADMIN;
   GRANT CREATE WAREHOUSE ON ACCOUNT TO INMSTRUCTOR1_DB_SYSADMIN;
...
-- +------------------------------------------------------------------------------------------------------+
-- | Try to use role INMSTRUCTOR1_DB_SYSADMIN                                                             |
-- +------------------------------------------------------------------------------------------------------+

USE ROLE INMSTRUCTOR1_DB_SYSADMIN;

-- +------------------------------------------------------------------------------------------------------+
-- | Create database INMSTRUCTOR1_DB                                                                      |
-- +------------------------------------------------------------------------------------------------------+

   CREATE DATABASE IF NOT EXISTS INMSTRUCTOR1_DB DATA_RETENTION_TIME_IN_DAYS = 1;
USE DATABASE INMSTRUCTOR1_DB;
...
-- +------------------------------------------------------------------------------------------------------+
-- | Try to use role INMSTRUCTOR1_DB_SECADMIN                                                             |
-- +------------------------------------------------------------------------------------------------------+

USE ROLE INMSTRUCTOR1_DB_SECADMIN;

-- +------------------------------------------------------------------------------------------------------+
-- | Create Account Level functional roles                                                                |
-- +------------------------------------------------------------------------------------------------------+

   CREATE ROLE IF NOT EXISTS INMSTRUCTOR1_DB_ELT_SVC;
   CREATE ROLE IF NOT EXISTS INMSTRUCTOR1_DB_ANALYST;
   CREATE ROLE IF NOT EXISTS INMSTRUCTOR1_DB_DATA_SCI;
...
USE ROLE INMSTRUCTOR1_DB_SECADMIN;
   CREATE  ROLE IF NOT EXISTS _INMSTRUCTOR1_DB_ELT_WH_WU;
   CREATE  ROLE IF NOT EXISTS _INMSTRUCTOR1_DB_ELT_WH_WFULL;
   CREATE  ROLE IF NOT EXISTS _INMSTRUCTOR1_DB_ELT_WH_ALL;
   CREATE  ROLE IF NOT EXISTS _INMSTRUCTOR1_DB_ANALYST_WH_WU;
   CREATE  ROLE IF NOT EXISTS _INMSTRUCTOR1_DB_ANALYST_WH_WFULL;
   CREATE  ROLE IF NOT EXISTS _INMSTRUCTOR1_DB_ANALYST_WH_ALL;
   CREATE  ROLE IF NOT EXISTS _INMSTRUCTOR1_DB_DATA_SCI_WH_WU;
   CREATE  ROLE IF NOT EXISTS _INMSTRUCTOR1_DB_DATA_SCI_WH_WFULL;
   CREATE  ROLE IF NOT EXISTS _INMSTRUCTOR1_DB_DATA_SCI_WH_ALL;

-- +------------------------------------------------------------------------------------------------------+
-- | Grant ownership on warehouse access roles to INMSTRUCTOR1_DB_SECADMIN                                |
-- +------------------------------------------------------------------------------------------------------+

   GRANT OWNERSHIP ON ROLE _INMSTRUCTOR1_DB_ELT_WH_WU TO ROLE INMSTRUCTOR1_DB_SECADMIN;
   GRANT OWNERSHIP ON ROLE _INMSTRUCTOR1_DB_ELT_WH_WFULL TO ROLE INMSTRUCTOR1_DB_SECADMIN;
   GRANT OWNERSHIP ON ROLE _INMSTRUCTOR1_DB_ELT_WH_ALL TO ROLE INMSTRUCTOR1_DB_SECADMIN;
   GRANT OWNERSHIP ON ROLE _INMSTRUCTOR1_DB_ANALYST_WH_WU TO ROLE INMSTRUCTOR1_DB_SECADMIN;
   GRANT OWNERSHIP ON ROLE _INMSTRUCTOR1_DB_ANALYST_WH_WFULL TO ROLE INMSTRUCTOR1_DB_SECADMIN;
   GRANT OWNERSHIP ON ROLE _INMSTRUCTOR1_DB_ANALYST_WH_ALL TO ROLE INMSTRUCTOR1_DB_SECADMIN;
   GRANT OWNERSHIP ON ROLE _INMSTRUCTOR1_DB_DATA_SCI_WH_WU TO ROLE INMSTRUCTOR1_DB_SECADMIN;
   GRANT OWNERSHIP ON ROLE _INMSTRUCTOR1_DB_DATA_SCI_WH_WFULL TO ROLE INMSTRUCTOR1_DB_SECADMIN;
   GRANT OWNERSHIP ON ROLE _INMSTRUCTOR1_DB_DATA_SCI_WH_ALL TO ROLE INMSTRUCTOR1_DB_SECADMIN;
...
-- +------------------------------------------------------------------------------------------------------+
-- | Grant account level functional roles to INMSTRUCTOR1_DB_SYSADMIN                                     |
-- +------------------------------------------------------------------------------------------------------+

   GRANT ROLE INMSTRUCTOR1_DB_ELT_SVC TO ROLE INMSTRUCTOR1_DB_SYSADMIN;
   GRANT ROLE INMSTRUCTOR1_DB_ANALYST TO ROLE INMSTRUCTOR1_DB_SYSADMIN;
   GRANT ROLE INMSTRUCTOR1_DB_DATA_SCI TO ROLE INMSTRUCTOR1_DB_SYSADMIN;

-- +------------------------------------------------------------------------------------------------------+
-- | Grant access roles to INMSTRUCTOR1_DB_SYSADMIN                                                       |
-- +------------------------------------------------------------------------------------------------------+

USE ROLE INMSTRUCTOR1_DB_SYSADMIN;
   GRANT DATABASE ROLE _RAW_RO TO ROLE INMSTRUCTOR1_DB_SYSADMIN;
   GRANT DATABASE ROLE _RAW_RW TO ROLE INMSTRUCTOR1_DB_SYSADMIN;
   GRANT DATABASE ROLE _RAW_SFULL TO ROLE INMSTRUCTOR1_DB_SYSADMIN;
   GRANT DATABASE ROLE _EDW_RO TO ROLE INMSTRUCTOR1_DB_SYSADMIN;
   GRANT DATABASE ROLE _EDW_RW TO ROLE INMSTRUCTOR1_DB_SYSADMIN;
   GRANT DATABASE ROLE _EDW_SFULL TO ROLE INMSTRUCTOR1_DB_SYSADMIN;
   GRANT DATABASE ROLE _MART_RO TO ROLE INMSTRUCTOR1_DB_SYSADMIN;
   GRANT DATABASE ROLE _MART_RW TO ROLE INMSTRUCTOR1_DB_SYSADMIN;
   GRANT DATABASE ROLE _MART_SFULL TO ROLE INMSTRUCTOR1_DB_SYSADMIN;

-- +------------------------------------------------------------------------------------------------------+
-- | Grant access roles to functional roles                                                               |
-- +------------------------------------------------------------------------------------------------------+

   GRANT DATABASE ROLE _RAW_RW to role INMSTRUCTOR1_DB_ELT_SVC;
   GRANT DATABASE ROLE _RAW_RW to role INMSTRUCTOR1_DB_DATA_SCI;
   GRANT DATABASE ROLE _EDW_RW to role INMSTRUCTOR1_DB_ELT_SVC;
   GRANT DATABASE ROLE _EDW_RO to role INMSTRUCTOR1_DB_ANALYST;
   GRANT DATABASE ROLE _EDW_RO to role INMSTRUCTOR1_DB_DATA_SCI;
   GRANT DATABASE ROLE _MART_RW to role INMSTRUCTOR1_DB_ELT_SVC;
   GRANT DATABASE ROLE _MART_RO to role INMSTRUCTOR1_DB_ANALYST;
   GRANT DATABASE ROLE _MART_RO to role INMSTRUCTOR1_DB_DATA_SCI;

-- +------------------------------------------------------------------------------------------------------+
-- | Grant warehouse access roles to INMSTRUCTOR1_DB_SYSADMIN                                             |
-- +------------------------------------------------------------------------------------------------------+

USE ROLE INMSTRUCTOR1_DB_SECADMIN;
   GRANT ROLE _INMSTRUCTOR1_DB_ELT_WH_WU TO ROLE INMSTRUCTOR1_DB_SYSADMIN;
   GRANT ROLE _INMSTRUCTOR1_DB_ELT_WH_WFULL TO ROLE INMSTRUCTOR1_DB_SYSADMIN;
   GRANT ROLE _INMSTRUCTOR1_DB_ELT_WH_ALL TO ROLE INMSTRUCTOR1_DB_SYSADMIN;
   GRANT ROLE _INMSTRUCTOR1_DB_ANALYST_WH_WU TO ROLE INMSTRUCTOR1_DB_SYSADMIN;
   GRANT ROLE _INMSTRUCTOR1_DB_ANALYST_WH_WFULL TO ROLE INMSTRUCTOR1_DB_SYSADMIN;
   GRANT ROLE _INMSTRUCTOR1_DB_ANALYST_WH_ALL TO ROLE INMSTRUCTOR1_DB_SYSADMIN;
   GRANT ROLE _INMSTRUCTOR1_DB_DATA_SCI_WH_WU TO ROLE INMSTRUCTOR1_DB_SYSADMIN;
   GRANT ROLE _INMSTRUCTOR1_DB_DATA_SCI_WH_WFULL TO ROLE INMSTRUCTOR1_DB_SYSADMIN;
   GRANT ROLE _INMSTRUCTOR1_DB_DATA_SCI_WH_ALL TO ROLE INMSTRUCTOR1_DB_SYSADMIN;

-- +------------------------------------------------------------------------------------------------------+
-- | Grant virtual warehouse access roles to functional roles                                             |
-- +------------------------------------------------------------------------------------------------------+

   GRANT ROLE _INMSTRUCTOR1_DB_ELT_WH_WU TO ROLE INMSTRUCTOR1_DB_ELT_SVC;
   GRANT ROLE _INMSTRUCTOR1_DB_ANALYST_WH_WU TO ROLE INMSTRUCTOR1_DB_ANALYST;
   GRANT ROLE _INMSTRUCTOR1_DB_DATA_SCI_WH_WU TO ROLE INMSTRUCTOR1_DB_DATA_SCI;
...
-- +------------------------------------------------------------------------------------------------------+
   ...
   GRANT USAGE ON DATABASE INMSTRUCTOR1_DB TO DATABASE ROLE _RAW_RO;
   GRANT USAGE ON DATABASE INMSTRUCTOR1_DB TO DATABASE ROLE _RAW_RW;
   GRANT USAGE ON DATABASE INMSTRUCTOR1_DB TO DATABASE ROLE _RAW_SFULL;
   GRANT USAGE ON DATABASE INMSTRUCTOR1_DB TO DATABASE ROLE _EDW_RO;
   GRANT USAGE ON DATABASE INMSTRUCTOR1_DB TO DATABASE ROLE _EDW_RW;
   GRANT USAGE ON DATABASE INMSTRUCTOR1_DB TO DATABASE ROLE _EDW_SFULL;
   GRANT USAGE ON DATABASE INMSTRUCTOR1_DB TO DATABASE ROLE _MART_RO;
   GRANT USAGE ON DATABASE INMSTRUCTOR1_DB TO DATABASE ROLE _MART_RW;
   GRANT USAGE ON DATABASE INMSTRUCTOR1_DB TO DATABASE ROLE _MART_SFULL;

-- +------------------------------------------------------------------------------------------------------+
-- | Create the warehouses                                                                                |
-- +------------------------------------------------------------------------------------------------------+

   CREATE WAREHOUSE IF NOT EXISTS INMSTRUCTOR1_DB_ELT_WH WITH WAREHOUSE_TYPE = 'STANDARD' warehouse_size='SMALL', scaling_policy='standard',min_cluster_count=1, max_cluster_count=1, auto_suspend=300,auto_resume=true, initially_suspended=true;
   CREATE WAREHOUSE IF NOT EXISTS INMSTRUCTOR1_DB_ANALYST_WH WITH WAREHOUSE_TYPE = 'STANDARD' warehouse_size='SMALL', scaling_policy='standard',min_cluster_count=1, max_cluster_count=1, auto_suspend=300,auto_resume=true, initially_suspended=true;
   CREATE WAREHOUSE IF NOT EXISTS INMSTRUCTOR1_DB_DATA_SCI_WH WITH WAREHOUSE_TYPE = 'STANDARD' warehouse_size='medium', scaling_policy='standard',min_cluster_count=1, max_cluster_count=1, auto_suspend=300,auto_resume=true, initially_suspended=true;

-- +------------------------------------------------------------------------------------------------------+
-- | Grant privileges on warehouses to access roles                                                       |
-- +------------------------------------------------------------------------------------------------------+

   GRANT USAGE ON WAREHOUSE INMSTRUCTOR1_DB_ELT_WH TO ROLE _INMSTRUCTOR1_DB_ELT_WH_WU;
   GRANT USAGE, OPERATE,MODIFY ON WAREHOUSE INMSTRUCTOR1_DB_ELT_WH TO ROLE _INMSTRUCTOR1_DB_ELT_WH_WFULL;
   GRANT ALL ON WAREHOUSE INMSTRUCTOR1_DB_ELT_WH TO ROLE _INMSTRUCTOR1_DB_ELT_WH_ALL;
   GRANT USAGE ON WAREHOUSE INMSTRUCTOR1_DB_ANALYST_WH TO ROLE _INMSTRUCTOR1_DB_ANALYST_WH_WU;
   GRANT USAGE, OPERATE,MODIFY ON WAREHOUSE INMSTRUCTOR1_DB_ANALYST_WH TO ROLE _INMSTRUCTOR1_DB_ANALYST_WH_WFULL;
   GRANT ALL ON WAREHOUSE INMSTRUCTOR1_DB_ANALYST_WH TO ROLE _INMSTRUCTOR1_DB_ANALYST_WH_ALL;
   GRANT USAGE ON WAREHOUSE INMSTRUCTOR1_DB_DATA_SCI_WH TO ROLE _INMSTRUCTOR1_DB_DATA_SCI_WH_WU;
   GRANT USAGE, OPERATE,MODIFY ON WAREHOUSE INMSTRUCTOR1_DB_DATA_SCI_WH TO ROLE _INMSTRUCTOR1_DB_DATA_SCI_WH_WFULL;
   GRANT ALL ON WAREHOUSE INMSTRUCTOR1_DB_DATA_SCI_WH TO ROLE _INMSTRUCTOR1_DB_DATA_SCI_WH_ALL;

-- +------------------------------------------------------------------------------------------------------+
-- | Create the schemas                                                                                   |
-- +------------------------------------------------------------------------------------------------------+

   CREATE SCHEMA IF NOT EXISTS INMSTRUCTOR1_DB.RAW WITH MANAGED ACCESS data_retention_time_in_days=1 comment = '';
   CREATE SCHEMA IF NOT EXISTS INMSTRUCTOR1_DB.EDW WITH MANAGED ACCESS data_retention_time_in_days=1 comment = '';
   CREATE SCHEMA IF NOT EXISTS INMSTRUCTOR1_DB.MART WITH MANAGED ACCESS data_retention_time_in_days=1 comment = '';
...
-- +------------------------------------------------------------------------------------------------------+
-- | Build of INMSTRUCTOR1_DB complete - have a nice day                                                  |
-- +------------------------------------------------------------------------------------------------------+
