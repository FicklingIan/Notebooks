...
-- +------------------------------------------------------------------------------------------------------+
   ...
   CREATE ROLE IF NOT EXISTS PROD_EDW_ELT_SVC;
...
USE ROLE PROD_EDW_SECADMIN;
   CREATE  ROLE IF NOT EXISTS _PROD_EDW_ELT_WH_WU;
   CREATE  ROLE IF NOT EXISTS _PROD_EDW_ELT_WH_WFULL;
   CREATE  ROLE IF NOT EXISTS _PROD_EDW_ELT_WH_ALL;
   CREATE  ROLE IF NOT EXISTS _PROD_EDW_ANALYST_WH_WU;
   CREATE  ROLE IF NOT EXISTS _PROD_EDW_ANALYST_WH_WFULL;
   CREATE  ROLE IF NOT EXISTS _PROD_EDW_ANALYST_WH_ALL;
   CREATE  ROLE IF NOT EXISTS _PROD_EDW_DATA_SCI_WH_WU;
   CREATE  ROLE IF NOT EXISTS _PROD_EDW_DATA_SCI_WH_WFULL;
   CREATE  ROLE IF NOT EXISTS _PROD_EDW_DATA_SCI_WH_ALL;

-- +------------------------------------------------------------------------------------------------------+
-- | Grant ownership on warehouse access roles to PROD_EDW_SECADMIN                                       |
-- +------------------------------------------------------------------------------------------------------+

   GRANT OWNERSHIP ON ROLE _PROD_EDW_ELT_WH_WU TO ROLE PROD_EDW_SECADMIN;
   GRANT OWNERSHIP ON ROLE _PROD_EDW_ELT_WH_WFULL TO ROLE PROD_EDW_SECADMIN;
   GRANT OWNERSHIP ON ROLE _PROD_EDW_ELT_WH_ALL TO ROLE PROD_EDW_SECADMIN;
   GRANT OWNERSHIP ON ROLE _PROD_EDW_ANALYST_WH_WU TO ROLE PROD_EDW_SECADMIN;
   GRANT OWNERSHIP ON ROLE _PROD_EDW_ANALYST_WH_WFULL TO ROLE PROD_EDW_SECADMIN;
   GRANT OWNERSHIP ON ROLE _PROD_EDW_ANALYST_WH_ALL TO ROLE PROD_EDW_SECADMIN;
   GRANT OWNERSHIP ON ROLE _PROD_EDW_DATA_SCI_WH_WU TO ROLE PROD_EDW_SECADMIN;
   GRANT OWNERSHIP ON ROLE _PROD_EDW_DATA_SCI_WH_WFULL TO ROLE PROD_EDW_SECADMIN;
   GRANT OWNERSHIP ON ROLE _PROD_EDW_DATA_SCI_WH_ALL TO ROLE PROD_EDW_SECADMIN;
...
-- +------------------------------------------------------------------------------------------------------+
   ...
   GRANT ROLE PROD_EDW_ELT_SVC TO ROLE PROD_EDW_SYSADMIN;
...
USE ROLE PROD_EDW_SECADMIN;
   GRANT ROLE _PROD_EDW_ELT_WH_WU TO ROLE PROD_EDW_SYSADMIN;
   GRANT ROLE _PROD_EDW_ELT_WH_WFULL TO ROLE PROD_EDW_SYSADMIN;
   GRANT ROLE _PROD_EDW_ELT_WH_ALL TO ROLE PROD_EDW_SYSADMIN;
   GRANT ROLE _PROD_EDW_ANALYST_WH_WU TO ROLE PROD_EDW_SYSADMIN;
   GRANT ROLE _PROD_EDW_ANALYST_WH_WFULL TO ROLE PROD_EDW_SYSADMIN;
   GRANT ROLE _PROD_EDW_ANALYST_WH_ALL TO ROLE PROD_EDW_SYSADMIN;
   GRANT ROLE _PROD_EDW_DATA_SCI_WH_WU TO ROLE PROD_EDW_SYSADMIN;
   GRANT ROLE _PROD_EDW_DATA_SCI_WH_WFULL TO ROLE PROD_EDW_SYSADMIN;
   GRANT ROLE _PROD_EDW_DATA_SCI_WH_ALL TO ROLE PROD_EDW_SYSADMIN;

-- +------------------------------------------------------------------------------------------------------+
-- | Grant virtual warehouse access roles to functional roles                                             |
-- +------------------------------------------------------------------------------------------------------+

   GRANT ROLE _PROD_EDW_ELT_WH_WU TO ROLE PROD_EDW_ELT_SVC;
   GRANT ROLE _PROD_EDW_ANALYST_WH_WU TO ROLE PROD_EDW_ANALYST;
   GRANT ROLE _PROD_EDW_DATA_SCI_WH_WU TO ROLE PROD_EDW_DATA_SCI;
...
-- +------------------------------------------------------------------------------------------------------+
   ...
   CREATE WAREHOUSE IF NOT EXISTS PROD_EDW_ELT_WH WITH WAREHOUSE_TYPE = 'STANDARD' warehouse_size='SMALL', scaling_policy='standard',min_cluster_count=1, max_cluster_count=1, auto_suspend=300,auto_resume=true, initially_suspended=true;
   CREATE WAREHOUSE IF NOT EXISTS PROD_EDW_ANALYST_WH WITH WAREHOUSE_TYPE = 'STANDARD' warehouse_size='SMALL', scaling_policy='standard',min_cluster_count=1, max_cluster_count=1, auto_suspend=300,auto_resume=true, initially_suspended=true;
   CREATE WAREHOUSE IF NOT EXISTS PROD_EDW_DATA_SCI_WH WITH WAREHOUSE_TYPE = 'STANDARD' warehouse_size='medium', scaling_policy='standard',min_cluster_count=1, max_cluster_count=1, auto_suspend=300,auto_resume=true, initially_suspended=true;

-- +------------------------------------------------------------------------------------------------------+
-- | Grant privileges on warehouses to access roles                                                       |
-- +------------------------------------------------------------------------------------------------------+

   GRANT USAGE ON WAREHOUSE PROD_EDW_ELT_WH TO ROLE _PROD_EDW_ELT_WH_WU;
   GRANT USAGE, OPERATE,MODIFY ON WAREHOUSE PROD_EDW_ELT_WH TO ROLE _PROD_EDW_ELT_WH_WFULL;
   GRANT ALL ON WAREHOUSE PROD_EDW_ELT_WH TO ROLE _PROD_EDW_ELT_WH_ALL;
   GRANT USAGE ON WAREHOUSE PROD_EDW_ANALYST_WH TO ROLE _PROD_EDW_ANALYST_WH_WU;
   GRANT USAGE, OPERATE,MODIFY ON WAREHOUSE PROD_EDW_ANALYST_WH TO ROLE _PROD_EDW_ANALYST_WH_WFULL;
   GRANT ALL ON WAREHOUSE PROD_EDW_ANALYST_WH TO ROLE _PROD_EDW_ANALYST_WH_ALL;
   GRANT USAGE ON WAREHOUSE PROD_EDW_DATA_SCI_WH TO ROLE _PROD_EDW_DATA_SCI_WH_WU;
   GRANT USAGE, OPERATE,MODIFY ON WAREHOUSE PROD_EDW_DATA_SCI_WH TO ROLE _PROD_EDW_DATA_SCI_WH_WFULL;
   GRANT ALL ON WAREHOUSE PROD_EDW_DATA_SCI_WH TO ROLE _PROD_EDW_DATA_SCI_WH_ALL;
